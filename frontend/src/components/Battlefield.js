    ...
    for (let row = 0; row < 3; row++) {
      for (let col = 0; col < 5; col++) {
        ...
        grid.push(
          ...
          >
            {/* Grid Coordinates */}
            <div className="absolute top-1 left-1 text-xs text-gray-400 font-mono">{position}</div>
            ...
            {unit && (
              ...
              >
                {/* Unit Icon */}
                <div className="text-2xl mb-1">{unit.type === 'commander' ? '👑' : '⚔️'}</div>

                {/* Unit Name */}
                <div className="text-xs font-bold truncate w-full px-1">{unit.name}</div>
    ...
    <div className="battlefield h-full flex flex-col justify-center items-center p-4 relative perspective-1000">
      ...
      <div className="absolute top-4 right-20 bg-black/70 backdrop-blur-sm border border-cyan-400/30 rounded-lg p-4">
        ...
        <div className="text-xs text-gray-300">
          Player Units:{' '}
          {Object.values(battlefieldUnits).filter((u) => u.player === 'player').length}/8
          <br />
          Enemy Units: {Object.values(battlefieldUnits).filter((u) => u.player === 'enemy').length}
          /8
        </div>
      </div>
    </div>
