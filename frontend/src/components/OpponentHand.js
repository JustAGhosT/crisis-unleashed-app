...
const OpponentHand = () => {
  ...
  const opponentCards = [{ id: 1 }, { id: 2 }, { id: 3 }, { id: 4 }, { id: 5 }, { id: 6 }];
    ...
    <div className="opponent-hand flex flex-col items-center justify-center h-full">
      ...
      <div className="mb-2">
        <div className="flex items-center space-x-2">
          <h3 className="text-red-400 font-bold font-mono text-xs">ENEMY HAND</h3>
          <div className="text-xs text-red-300/60 font-mono">[{opponentCards.length}]</div>
        </div>
      </div>
      ...
      <div className="mt-2">
        <div className="flex items-center space-x-2">
          <div className="text-xs text-red-400 font-mono">THREAT:</div>
          <div className="flex space-x-0.5">
            {[...Array(5)].map((_, i) => (
              <div
                key={i}
                className={`w-1.5 h-1.5 rounded-sm ${
                  i < 3 ? 'bg-red-400 shadow-red-400/50' : 'bg-gray-600'
                }`}
              />
            ))}
          </div>
