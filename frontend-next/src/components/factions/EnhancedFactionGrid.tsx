...
type FilterOption = 'all' | FactionId | 'energy' | 'stealth' | 'time' | 'mind' | 'adaptation' | 'sacrifice';
...
export function EnhancedFactionGrid({ 
  ...
  const filteredFactions = useMemo(() => {
    return factions.filter(faction => {
      ...
      // Filter by mechanics
      if (filterBy === 'energy' && faction.mechanics.energyManipulation) return true;
      ...
      if (filterBy === 'adaptation' && faction.mechanics.adaptation) return true;
  ...
  const cardStyle = isNewThemeEnabled && isDark
    ...
    <div className="space-y-6">
      <Card className={`${cardStyle} transition-colors duration-200`}>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            {/* Search input */}
            <div>
              <Label htmlFor="search" className="mb-2 block dark:text-gray-200">Search</Label>
              <Input
                id="search"
            ...
            {/* Sort select */}
            <div>
              <label htmlFor="sort" className="mb-2 block dark:text-gray-200">Sort By</label>
              <select
                id="sort"
                value={sortBy}
            ...
            {/* Filter select */}
            <div>
              <label htmlFor="filter" className="mb-2 block dark:text-gray-200">Filter By</label>
              <select
                id="filter"
                value={filterBy}
      ...
      {sortedFactions.length > 0 ? (
        ...
        <div className="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">No factions found</h3>
