...
export default function UserProfile() {
  ...
  const experiencePercentage = Math.round(
    (profile.experience.current / profile.experience.next) * 100,
  );
    ...
    <div className="bg-white shadow rounded-lg p-6">
      <div className="flex flex-col items-center">
        ...
        <div className="mt-4 w-full">
          ...
          >
            <div
              className={`bg-blue-600 h-2.5 rounded-full ${styles.xpBar}`}
            ></div>
          </div>
          <p className="text-xs text-gray-500 mt-1 text-center">
            {profile.experience.next - profile.experience.current} XP to level{" "}
            {profile.level + 1}
          </p>
        </div>
        ...
        <div className="mt-4 w-full">
          <p className="text-sm font-medium text-gray-700">
            Preferred Faction:{" "}
            <span className="font-semibold">{profile.preferredFaction}</span>
          </p>
        </div>
