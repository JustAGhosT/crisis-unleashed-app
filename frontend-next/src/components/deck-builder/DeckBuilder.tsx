...
import { RealtimeDevPanel } from "./dev/RealtimeDevPanel";
...
export const DeckBuilder: React.FC<DeckBuilderProps> = ({
  ...
  const { validation, stats } = useDeckValidation(
    availableCards,
    currentDeckCards,
  );
  ...
  const handleAddCard = React.useCallback((card: GameCardData) => {
    setCurrentDeckCards((previous) => {
      const existingCardIndex = previous.findIndex(
  ...
  // Optional: handle reorder from DeckList
  const handleReorderDeckCards = React.useCallback((newOrder: DeckCard[]) => {
    setCurrentDeckCards(newOrder);
  }, []);
  ...
  const handleSaveDeck = React.useMemo(
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            {/* Realtime Dev Panel (feature-flagged) */}
            <RealtimeDevPanel />
            {/* Deck Creation Form */}
                        ...
                        render={({ field }) => (
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              field.onChange(
                                value === "" ? undefined : (value as FactionId),
                              );
                            }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {availableCards.map((card) => (
                      <GameCard
                        ...
                        size="md"
                        disabled={
                          !ownedCardIds.has(card.id) && cardFilters.owned
                        }
                        draggable={true}
                      />
                    ))}
                ...
                {cardSearchResult && cardSearchResult.total > 20 && (
                  <div className="mt-6 flex justify-center gap-2">
                    ...
                    <span className="flex items-center px-4 text-sm text-gray-400">
                      Page {currentPage} of{" "}
                      {Math.ceil(cardSearchResult.total / 20)}
                    </span>
                    <Button
                      variant="outline"
                      onClick={() => setCurrentPage((p) => p + 1)}
                      disabled={
                        currentPage >= Math.ceil(cardSearchResult.total / 20)
                      }
                    >
                      Next
                    </Button>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <DeckList
              ...
              onReorderDeckCards={handleReorderDeckCards}
            />

            {/* Deck Statistics */}
            <DeckStats stats={stats} validation={validation} />
          </div>
        </div>
      </div>
    </div>
...
// RealtimeDevPanel now imported from './dev/RealtimeDevPanel'
