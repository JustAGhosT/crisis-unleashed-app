...
export default function DeckStatsPanel({ deck }: DeckStatsPanelProps) {
  ...
  const averageCost = useMemo(() => {
    ...
    const totalCost = cardsWithCost.reduce((sum, card) => sum + (card.cost || 0), 0);
    return (totalCost / cardsWithCost.length).toFixed(2);
  }, [deck.cards]);
  
  // Calculate average power
  const averagePower = useMemo(() => {
    const cardsWithPower = deck.cards.filter(card => card.power !== undefined);
    if (cardsWithPower.length === 0) return 0;
    
    const totalPower = cardsWithPower.reduce((sum, card) => sum + (card.power || 0), 0);
      ...
      <CardContent>
        {deck.cards.length === 0 ? (
          <div className="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">No cards in deck</h3>
            <p className="mt-2 text-gray-500 dark:text-gray-400">Add cards to see deck statistics</p>
          </div>
        ) : (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Unique Cards</h3>
                <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">
              ...
              <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Avg. Cost</h3>
                <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">
                  {averageCost}
                </p>
              </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Avg. Power</h3>
                <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">
                  {averagePower}
                </p>
              </div>
            ...
            <div>
              <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-4">Cost Distribution</h3>
              <div className="h-64">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {/* Type distribution */}
              <div>
                <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-4">Card Types</h3>
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
              ...
              {/* Rarity distribution */}
              <div>
                <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-4">Card Rarities</h3>
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
