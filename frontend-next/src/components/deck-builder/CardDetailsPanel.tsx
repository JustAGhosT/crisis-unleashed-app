    ...
    return (
      <UICard className="bg-slate-800/50 border-slate-600 h-full">
        <CardHeader>
          ...
          <CardDescription className="text-gray-300">Select a card to view details</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-center text-sm text-gray-400 py-12">No card selected</div>
        </CardContent>
      </UICard>
    );
  }
      ...
      <CardHeader>
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <Badge className={cn('text-xs', rarityBadgeClass(card.rarity))}>{card.rarity}</Badge>
      ...
      <CardContent>
        <div className="grid grid-cols-1 gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-200 whitespace-pre-line">{card.description || '—'}</p>
          </div>

          <div className="grid grid-cols-3 gap-3">
            {typeof card.attack === 'number' && (
              <div className="text-center">
                <div className="text-lg font-semibold text-red-400">{card.attack}</div>
                <div className="text-xs text-gray-400">Attack</div>
              </div>
            )}
            {typeof card.health === 'number' && (
              <div className="text-center">
                <div className="text-lg font-semibold text-green-400">{card.health}</div>
                <div className="text-xs text-gray-400">Health</div>
              </div>
            )}
            {card.keywords && card.keywords.length > 0 && (
              <div className="text-center">
                <div className="text-xs text-gray-300">{card.keywords.join(', ')}</div>
