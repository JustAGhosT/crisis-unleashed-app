...
jest.mock("../GameCard", () => ({
  GameCard: ({
    ...
    card: GameCardData;
    onClick?: (c: GameCardData) => void;
    onAdd?: (c: GameCardData) => void;
    onRemove?: (c: GameCardData) => void;
    quantity?: number;
    maxQuantity?: number;
    disabled?: boolean;
  }) => (
    ...
    >
      ...
      <button
        ...
        data-testid={`card-remove-${card.id}`}
      >
        Remove Card
      </button>
    </div>
...
describe("CardGrid Component", () => {
  ...
  test("passes quantity values to GameCard components", () => {
    ...
    expect(screen.getByTestId("game-card-card-1")).toHaveAttribute(
      "data-quantity",
      "2",
    );
    expect(screen.getByTestId("game-card-card-2")).toHaveAttribute(
      "data-quantity",
      "1",
    );
    expect(screen.getByTestId("game-card-card-3")).toHaveAttribute(
      "data-quantity",
      "0",
    );
  ...
  test("passes disabled state to GameCard components", () => {
    render(
      <CardGrid cards={mockCards} disabledCardIds={["card-1", "card-3"]} />,
    );
  ...
  test("applies correct grid column classes based on columnCount prop", () => {
    const { container, rerender } = render(
      <CardGrid cards={mockCards} columnCount={2} />,
    );
  ...
  test("applies custom className", () => {
    const { container } = render(
      <CardGrid cards={mockCards} className="custom-class" />,
    );
