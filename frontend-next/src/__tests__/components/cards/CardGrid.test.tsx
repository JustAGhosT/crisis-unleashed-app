...
describe("CardGrid Component", () => {
  ...
  it("renders loading state", () => {
    const { container } = render(
      <CardGrid cards={[]} loading={true} onCardClick={mockCardClick} />,
    );
  ...
  it("calls click handler when card is clicked", () => {
    render(<CardGrid cards={mockCards} onCardClick={mockCardClick} />);
  ...
  it("renders with different column counts", () => {
    const { rerender } = render(<CardGrid cards={mockCards} columnCount={2} />);
