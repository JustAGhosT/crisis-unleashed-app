 ...
 */
export function useDeck({ initialDeck, userId, factionId }: UseDeckProps = {}) {
  ...
  const initNewDeck = useCallback(
    (name: string, faction: FactionId) => {
      if (!userId) {
  ...
  const addCard = useCallback(
    (card: Card) => {
      ...
      setDeck((currentDeck) => {
        ...
        const existingCardIndex = currentDeck.cards.findIndex(
  ...
  useEffect(() => {
    ...
    if (totalCards < 40) {
      errors.push(
        `Deck must contain at least 40 cards (currently ${totalCards})`,
      );
    }
    if (totalCards > 60) {
      errors.push(
        `Deck cannot contain more than 60 cards (currently ${totalCards})`,
      );
    }
    ...
    const nonFactionCards = deckCards.filter(
      (dc) => dc.card.faction !== deck.faction,
    );
    const offFactionCopies = nonFactionCards.reduce(
      (s, dc) => s + dc.quantity,
      0,
    );
    ...
    const cardTypes = deckCards.reduce(
      (acc, dc) => {
    ...
    const costCurve = deckCards.reduce(
      (acc, dc) => {
    ...
    if (lowCostCards < totalCards * 0.3) {
      warnings.push(
        "Your deck has few low-cost cards, which may lead to slow starts",
      );
    }
